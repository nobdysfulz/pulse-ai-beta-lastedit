...
export default function AgentIntelligenceTab() {
    const { user, agentConfig, agentProfile, preferences, refreshUserData, loading } = useContext(UserContext);
    ...
    return (
        <div className="space-y-6">
            ...
            <Card className="bg-white">
                ...
                <CardContent>
                    <IntelligenceProfileSection title="Experience & Business" profile={agentProfile} fields={['experienceLevel', 'workCommitment', 'businessStructure', 'workSchedule']} />
                    <IntelligenceProfileSection title="Performance & Database" profile={agentProfile} fields={['previousYearTransactions', 'previousYearVolume', 'databaseSize', 'sphereWarmth', 'averagePricePoint']} />
                    <IntelligenceProfileSection title="Challenges & Growth" profile={agentProfile} fields={['businessConsistency', 'biggestChallenges', 'growthTimeline', 'learningPreference']} />
                </CardContent>
            </Card>
        </div>
    );
...
const IntelligenceProfileSection = ({ title, profile, fields }) => {
    const isComplete = profile ? fields.some(field => {
