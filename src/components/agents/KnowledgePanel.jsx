...
export default function KnowledgePanel() {
  ...
  const loadKnowledge = async () => {
    setLoading(true);
    try {
      const items = await base44.entities.UserKnowledge.filter({ userId: user.id }, '-created_date');
      setKnowledge(items || []);
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingItem) {
        const updated = await base44.entities.UserKnowledge.update(editingItem.id, formData);
        // Update in list immediately
        setKnowledge(prev => prev.map(item => item.id === editingItem.id ? updated : item));
        toast.success('Knowledge updated');
      } else {
        const created = await base44.entities.UserKnowledge.create({
          userId: user.id,
          ...formData
        });
  ...
  const handleDelete = async (itemId) => {
    try {
      await base44.entities.UserKnowledge.delete(itemId);
      // Remove from list immediately
      setKnowledge(prev => prev.filter(item => item.id !== itemId));
