...
const IconLink = ({ href, page, external, icon: Icon, title }) => {
  ...
  return (
    <Link to={createPageUrl(page)} aria-label={title}>
      {content}
    </Link>
  );
};
...
const OpenNowButton = () => (
  <div className="inline-flex items-center gap-2 px-4 py-2 bg-slate-900 text-white text-xs font-bold rounded-full group-hover:bg-slate-800 transition-colors">
    Open Now
    <ArrowRight className="w-4 h-4 text-purple-400" aria-hidden="true" />
  </div>
);
...
const ActionCard = ({ href, title }) => (
  <a
    ...
    className="bg-gradient-to-br pt-24 pr-4 pb-4 pl-4 relative block rounded-2xl from-indigo-100/70 to-fuchsia-100/70 flex justify-between items-end group transition-all duration-300 shadow-s"
...
export default function QuickActions() {
  ...
  return (
    <nav className="flex flex-col gap-4 h-full" aria-label="Quick actions">
      ...
      <a
        ...
        aria-label="View upcoming live trainings and masterminds"
      >
        <h3 className="text-2xl text-xl font-semibold leading-tight">
          View our Upcoming Live Trainings & Masterminds
        </h3>
        <div className="mt-4">
          <OpenNowButton />
        </div>
